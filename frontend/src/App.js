...
function App() {
  const [authed, setAuthed] = useState(
    () => !!sessionStorage.getItem("ajt_token")
  );
  const [currentUser, setCurrentUser] = useState(
    () => sessionStorage.getItem("ajt_user") || ""
  );
  ...
  const handleLogout = () => {
    sessionStorage.removeItem("ajt_token");
    sessionStorage.removeItem("ajt_user");
    setAuthed(false);
    setCurrentUser("");
  };
  ...
  return (
    <div className="App">
      ...
      <ResumeProvider>
        <BrowserRouter>
          <Routes>
            <Route element={<AppLayout onLogout={handleLogout} currentUser={currentUser} />}>
              ...
              <Route path="/jobs/ignored" element={<JobsPage key="ignored" filterKey="status" filterValue="ignored" title="Ignored" subtitle="Jobs you've skipped — hidden from all other views" />} />
              <Route path="/jobs/linkedin" element={<JobsPage key="linkedin" filterKey="platform" filterValue="linkedin" title="LinkedIn (India)" subtitle="Best-effort scrape of LinkedIn India guest listings." />} />
              <Route path="/jobs/aws" element={<JobsPage key="aws" filterKey="tag" filterValue="aws" title="AWS / Cloud Jobs" subtitle="DevOps, SRE, and Architect roles that involve AWS" />} />
